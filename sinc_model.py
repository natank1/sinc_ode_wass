...
import math
import torch_for_privae.sinc_conf as conf
...
class SincNet(nn.Module):
    ...
    def __init__(self,input_dim):
        ...
        self.cnn_N_filt = conf.cnn_N_filt
        self.cnn_len_filt = conf.cnn_len_filt
        self.cnn_max_pool_len = conf.cnn_max_pool_len

        self.cnn_act = conf.cnn_act
        self.cnn_drop = conf.cnn_drop

        self.cnn_use_laynorm = conf.cnn_use_laynorm
        self.cnn_use_batchnorm = conf.cnn_use_batchnorm
        self.cnn_use_laynorm_inp = conf.cnn_use_laynorm_inp
        self.cnn_use_batchnorm_inp = conf.cnn_use_batchnorm_inp

        self.input_dim = input_dim

        self.fs = 8000

        self.N_cnn_lay = len(conf.cnn_N_filt)
...
if __name__ =="__main__":
    ...
    inp0= 40000
    mm =SincNet(inp0).to(device)
    vv=torch.tensor( np.random.rand(1,inp0)).float().to(device)
    y= mm(vv)
    print ("hhhh",y.shape)